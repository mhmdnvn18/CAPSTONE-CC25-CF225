// IllDetect Service Worker  
const CACHE_NAME = 'illdetect-v1.0.3';
const urlsToCache = [
    '/',
    '/index.html',
    '/manifest.json',
    // Vite will bundle files with hashed names, so we'll cache them dynamically
];
...
self.addEventListener('push', (event) => {
    ...
    event.waitUntil(
        self.registration.showNotification('IllDetect', options)
    );
});
...
async function syncPredictionData() {
    try {
        ...
        console.log('Service Worker: Syncing IllDetect prediction data');
